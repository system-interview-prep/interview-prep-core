...
async def delete_version(version:str,_:dict=Depends(require_admin),db:AsyncSession=Depends(get_db))->dict:return await TaxonomyAdminService(db).delete_version(version)
...
@router.delete("/versions/{version}/concepts/{concept_id}")
async def delete_concept(version:str,concept_id:str,_:dict=Depends(require_admin),db:AsyncSession=Depends(get_db))->dict:return await TaxonomyAdminService(db).delete_concept(version,concept_id)
