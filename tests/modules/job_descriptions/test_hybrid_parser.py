...
async def test_hybrid_parser_accepts_only_exactly_grounded_claims() -> None:
    candidate = {
        ...
        "requirements": [{"value": "Python", "quote": "Python experience is required.", "kind": "skill", "priority": "must_have"}],
    ...
    assert all(ref in evidence for item in [*parsed.responsibilities, *parsed.requirements, *parsed.benefits] for ref in item.evidence_refs)
...
async def test_hybrid_parser_uses_openai_facade_by_default(monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.modules.job_descriptions.parsing.hybrid.get_settings", lambda: SimpleNamespace(
        openai_api_key="test-key", jd_parser_max_output_tokens=768
    ))
